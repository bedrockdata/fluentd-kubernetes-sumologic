...
RUN [ -f /bin/entrypoint.sh ] && /bin/entrypoint.sh echo || : && \
    ...
    fluent-gem install fluent-plugin-s3 -v 1.1.4 && \
    ...
    fluent-gem install fluent-plugin-rewrite-tag-filter -v 2.1.0 && \
    fluent-gem install fluent-plugin-prometheus -v 1.1.0 && \
